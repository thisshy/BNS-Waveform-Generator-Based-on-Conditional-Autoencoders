...
amp_norm   = amp_model .predict(X_test_normalized, verbose=0)  
phase_norm = phase_model.predict(X_test_normalized, verbose=0) 


amp_rec   = scaler_y_amplitude  .inverse_transform(amp_norm)    
phase_rec = scaler_y_phase.inverse_transform(phase_norm)   



m_hp            = np.empty(n, dtype=np.float32)
o_hp            = np.empty(n, dtype=np.float32)
m_hc            = np.empty(n, dtype=np.float32)
o_hc            = np.empty(n, dtype=np.float32)
...
out_path = ('/path/to/your/data/res_mlp_cnn_hp_hc_metrics.npz')
np.savez(
    out_path,
    x_test         = X_test,
    mismatch_hp     = m_hp,
