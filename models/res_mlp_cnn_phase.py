...
y_phase_normalized = scaler_phase.fit_transform(y_phase)


seq_len   = y_phase_normalized.shape[1]
...
model = build_waveform_net(label='phase_res_mlp_cnn',param_dim=param_dim, seq_len=seq_len,mlp_units=1024,mlp_blocks=8,cnn_filters=32,cnn_blocks=3)
...
callbacks_phase = [reduce_lr, early_stop]
history = model.fit(
    X_train_normalized,                 
    y_phase_normalized,            
    epochs=epochs,
...
model.save('/path/to/your/model/res_mlp_cnn_phase', save_format='tf')
...
np.save(
    '/path/to/your/data/res_mlp_cnn_losses_phase.npy',
    losses
)
