...
data_len = y_amplitude.shape[1]    
para_len = X_train.shape[1]   
N_total  = X_train.shape[0]   

X_train_normalized, y_amplitude_normalized, scaler_X, scaler_y = cal_normalize_data(
    x_list=X_train,
    y=y_amplitude,
    x_scalers=MinMaxScaler(),
    y_scaler=StandardScaler(),
    print_name_list='amplitude'
)

train_steps, val_steps, train_dataset, val_dataset =cal_prepare_datasets(
    x_norm=[y_amplitude_normalized,X_train_normalized],
    y_norm=y_amplitude_normalized,
    save_dir="/path/to/your/data/blocks_amplitude",
    train_fraction=0.9,
    val_fraction=0.1,
    N_blocks=10,
    batch_size=128
...
autoencoder_amplitude, encoder_params_amplitude, encoder_amplitude, decoder_amplitude = cAE_model(data_len, para_len, 'amplitude')
autoencoder_amplitude.summary()
autoencoder_amplitude.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=initial_lr))


checkpoint_filepath = ('/path/to/your/data/amplitude_checkpoint.h5')
if os.path.exists(checkpoint_filepath):
    print(f"Found checkpoint at {checkpoint_filepath}. Loading weights...")
    autoencoder_amplitude.load_weights(checkpoint_filepath)
else:
...
callbacks_amplitude = [reduce_lr, es, checkpoint_cb]

history = autoencoder_amplitude.fit(
    train_dataset,
...
np.save(
    '/path/to/your/data/cAE_losses_amplitude.npy',
    losses
)
